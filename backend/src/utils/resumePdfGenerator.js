...
// Hinglish: yeh utility improved resume text ko professionally formatted PDF me convert karti hai.
...
// Hinglish: PDF ke layout constants — margins, font sizes, line heights sab yahan define hain.
...
// Hinglish: resume text ko sections me parse karta hai.
...
function parseResumeIntoSections(resumeText) {
  // Hinglish: common section headers ko match karne ke liye regex — case insensitive.
  ...
  // Hinglish: line by line parse karo — jab naya section header mile toh purana save karo aur naya shuru karo
  for (const line of lines) {
    const match = line.trim().match(sectionPattern);

    if (match) {
      // Hinglish: pichle section ko save karo agar kuch content hai
  ...
  // Hinglish: last section ko bhi save karo
  if (currentSection !== null) {
    sections.push({
      title: currentSection,
      content: contentLines.join("\n").trim(),
  ...
  // Hinglish: agar koi standard section nahi mila toh pura text ek "CONTENT" section me daal do.
...
// Hinglish: section ka title clean aur readable banata hai.
...
// Hinglish: bullet points aur list items ko parse karta hai.
...
function parseContentLines(content) {
  ...
  return content
    ...
    .map((line) => {
      // Hinglish: bullet point strip karo — dash, asterisk, dot sab hata do
      const bulletStripped = line.replace(/^[-•*▪▸►]\s*/, "").trim();

      // Hinglish: [GAP: ...] marker detect karo — yeh red me render hoga PDF me
...
// Hinglish: MAIN FUNCTION — improved resume text leke PDF Buffer return karta hai.
...
export function generateResumePdf(improvedResumeText, candidateName = "") {
  return new Promise((resolve, reject) => {
    try {
      // Hinglish: naya PDF document banao — US Letter size, auto pages enabled
      const doc = new PDFDocument({
        ...
        // Hinglish: bufferPages true se pehle saare pages memory me banenge, phir ek saath flush hoge
        bufferPages: true,
      });

      // Hinglish: PDF output ko chunks me collect karo — end pe Buffer.concat se final buffer banao
      const chunks = [];
      doc.on("data", (chunk) => chunks.push(chunk));
      doc.on("end", () => resolve(Buffer.concat(chunks)));
      doc.on("error", (error) => reject(error));

      // Hinglish: agar candidate ka naam diya hai toh PDF ke top pe bada bold naam likho
      if (candidateName && candidateName !== "User" && candidateName !== "Candidate") {
        doc
      ...
      // Hinglish: resume text ko sections me parse karo
      const sections = parseResumeIntoSections(improvedResumeText);

      // Hinglish: har section ko render karo — heading, line, content
      sections.forEach((section, sectionIndex) => {
        // Hinglish: pehle section ke upar extra spacing nahi chahiye
        if (sectionIndex > 0) {
          doc.moveDown(0.6);
        }

        // Hinglish: check karo ki page pe enough space hai section heading ke liye.
        ...
        // Hinglish: section heading render karo — bold, slightly larger font
        ...
        // Hinglish: heading ke neeche ek colored line draw karo — professional look ke liye
        const lineY = doc.y + 2;
        doc
        ...
        // Hinglish: section ka content parse karo aur har line render karo
        const contentItems = parseContentLines(section.content);

        contentItems.forEach((item) => {
          // Hinglish: page overflow check — agar line page ke bottom se neeche jaayegi toh naya page
          if (doc.y + 20 > doc.page.height - LAYOUT.margin) {
            doc.addPage();
          }

          if (item.isGap) {
            // Hinglish: [GAP] markers red me render karo — yeh genuine skill gaps hain
            doc
            ...
            // Hinglish: bullet points ko indent ke saath render karo — bullet symbol + text
            ...
            // Hinglish: bullet symbol ke baad actual text — thoda right me offset
            doc
              .font(LAYOUT.fonts.body)
              .fontSize(LAYOUT.sizes.body)
              .fillColor(LAYOUT.colors.darkGray)
            ...
            // Hinglish: normal text — no bullet, no indent
            doc
              .font(LAYOUT.fonts.body)
              .fontSize(LAYOUT.sizes.body)
              .fillColor(LAYOUT.colors.darkGray)
      ...
      // Hinglish: PDF finalize karo — saare pages flush ho jayenge aur "end" event fire hoga
      doc.end();
    } catch (error) {
      reject(error);
    }
