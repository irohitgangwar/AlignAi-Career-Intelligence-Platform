...
// yeh JSON shape instructions hain jo AI ko batati hain ki output ka exact format kya hona chahiye.
...
// interview questions ka JSON shape — yeh pehle se sahi tha, koi change nahi
...
// retrieval query banane ke liye helper — sabhi relevant text ko combine karta hai
...
// har GenAI call se pehle Pinecone se relevant memory chunks nikaalte hain.
...
// candidate ka compact profile summary banata hai prompt me inject karne ke liye.
...
// messages array ke end me JSON format instructions append karta hai.
...
// AI ka raw text response leke usme se JSON object extract karta hai.
...
// AI ko call karta hai, JSON instructions append karta hai, response parse karta hai,
...
// MAIN ANALYSIS FUNCTION — resume vs JD comparison karta hai.
...
export async function analyzeResumeVsJob(
  ...
  // Pinecone se relevant memory chunks nikaalo — resume, JD, past analyses
  ...
  // prompt template me saari values inject karo
  ...
  // AI se structured JSON response lo aur Zod schema se validate karo
  ...
  // analysis result ke saath RAG metadata bhi return karo — debugging ke liye useful hai
  return {
    ...analysis,
    ragMeta: {
...
// RESUME REWRITE FUNCTION — analysis ke suggestions ke basis pe resume improve karta hai.
...
export async function generateImprovedResume(
  ...
  // suggestions ko string format me convert karo — agar structured objects hain toh readable banao
  const suggestionsText = (improvementSuggestions || [])
    .map((suggestion) => {
      // agar suggestion ek object hai (new schema), toh readable format me convert karo
      ...
      // agar purana plain string format hai, toh as-is use karo (backward compatibility)
      return String(suggestion);
    })
    .join("\n");

  // Pinecone se relevant context nikaal rahe hain — suggestions bhi query me daale hain
  const ragChunks = await getRagChunks({
    ...options,
    resumeText,
    jobDescription,
  ...
  // rewrite prompt me saari values inject karo
  ...
  // structured JSON resume generate aur validate karo
...
// MOCK INTERVIEW FUNCTION — JD-specific aur resume-aware interview questions generate karta hai.
...
export async function generateMockInterviewQuestions(
  ...
  // interview context ke liye bhi Pinecone se retrieval kar rahe hain
  const ragChunks = await getRagChunks({
    ...options,
    resumeText,
    jobDescription,
  ...
  // structured JSON response leke Zod se validate karo
  return invokeStructuredModel(
    prompt,
    mockInterviewSchema,
...
// SKILL DEVELOPMENT PLAN FUNCTION — vulnerabilities ko ordered learning roadmap me convert karta hai.
...
export async function generateSkillDevelopmentPlan(
  ...
  // past analysis memory reuse kar rahe hain taaki plan consistent rahe
  const ragChunks = await getRagChunks({
    ...options,
  ...
  // plain text roadmap return karta hai
