// Hinglish: yeh Dashboard component hai — analysis ke results yahan render hote hain.
...
// Hinglish: React hooks import — useEffect page load pe data load karne ke liye,
// useState har dynamic value (loading state, active tab, etc.) track karne ke liye.
import React, { useEffect, useState } from "react";

// Hinglish: useNavigate — programmatically page switch karne ke liye (jaise Intake pe redirect)
import { useNavigate } from "react-router-dom";

// Hinglish: lucide-react ke icons import — har icon specific UI element ke liye hai.
// Target = company/role indicator, AlertTriangle = gaps warning, TrendingUp = score status,
// CheckCircle2 = strong points, Download/Copy/FileText = export buttons,
// ShieldAlert = interview risk warning, ArrowRight = before→after arrow,
// MessageSquareWarning = suggestion warning, Tag = keyword/section tag, BookOpen = summary
import {
  Target,
  AlertTriangle,
  TrendingUp,
...
// Hinglish: backend ka base URL — development me localhost:5000, production me env variable se aayega.
// import.meta.env Vite ka tarika hai environment variables access karne ka (CRA me process.env hota hai).
const API_BASE = import.meta.env.VITE_API_BASE_URL || "http://localhost:5000";

export default function Dashboard() {
  // Hinglish: useNavigate hook — redirect karne ke liye use hota hai (jaise /Intakee pe bhejne ke liye)
  const navigate = useNavigate();

  // Hinglish: analysisData — localStorage se loaded analysis result (matchScore, vulnerabilities, suggestions, etc.)
  // null matlab abhi load nahi hua ya data nahi mila
  const [analysisData, setAnalysisData] = useState(null);

  // Hinglish: isLoading — page pehli baar load ho raha hai, data fetch ho raha hai localStorage se
  const [isLoading, setIsLoading] = useState(true);

  // Hinglish: improvedResume — AI-generated improved resume text (empty matlab abhi generate nahi hua)
  const [improvedResume, setImprovedResume] = useState("");

  // Hinglish: isGenerating — improved resume generate ho raha hai (AI API call chal rahi hai)
  // true hone pe "Generating..." spinner dikhta hai button pe
  const [isGenerating, setIsGenerating] = useState(false);

  // Hinglish: isDownloadingPdf — PDF generate ho rahi hai backend pe (download ho raha hai)
  // true hone pe "Generating PDF..." spinner dikhta hai
  const [isDownloadingPdf, setIsDownloadingPdf] = useState(false);

  // Hinglish: activeTab — kaun sa tab selected hai (overview, gaps, suggestions, improve)
  const [activeTab, setActiveTab] = useState("overview");

  // Hinglish: copySuccess — clipboard copy success feedback ke liye (2 second ke liye "Copied!" dikhata hai)
  const [copySuccess, setCopySuccess] = useState(false);

  // Hinglish: page load pe localStorage se analysis data load karo.
  ...
  // Hinglish: improved resume generate karne ka handler.
  ...
  const handleGenerateImprovedResume = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/analysis/improve-resume`, {
        method: "POST",
      ...
      setImprovedResume(response.data.improvedResumeText);

      // Hinglish: agar userId hai toh improved resume ko profile me bhi save kar do
      if (userId) {
        await fetch(`${API_BASE}/api/profile/${userId}/improved-resume`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            improvedResumeText: response.data.improvedResumeText,
          }),
        });
      }
    } catch (error) {
  ...
  // Hinglish: PDF download handler — backend se PDF generate karwa ke browser me download trigger karta hai.
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/analysis/download-resume-pdf`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      // Hinglish: response ko blob me convert karo aur browser download trigger karo
  ...
  // Hinglish: plain text download — fallback option agar PDF nahi chahiye
  ...
  // Hinglish: clipboard me copy karo aur 2 second ke liye success feedback dikhao
  ...
  // Hinglish: naya analysis start karne ke liye purana data clear karo aur Intake page pe jao
  const handleNewAnalysis = () => {
    localStorage.removeItem("analysisData");
    navigate("/Intakee");
  };

  // Hinglish: severity ke hisaab se color classes return karta hai — High=red, Medium=amber, Low=blue
  ...
  return (
    <div className="min-h-screen bg-[#202020] text-white p-6 md:p-10">
      <div className="max-w-7xl mx-auto">
        {/* Hinglish: page header — company name, job title, aur action buttons */}
        ...
        {/* Hinglish: tab navigation — Overview, Gaps, Suggestions, Improve Resume */}
        ...
        {/* ============ OVERVIEW TAB ============ */}
        {/* Hinglish: overview tab — match score, match summary, aur strong points dikhata hai */}
        {activeTab === "overview" && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* Hinglish: circular progress bar — match score percentage dikhata hai */}
            ...
            {/* Hinglish: match summary card — AI ka overall assessment ek paragraph me */}
            ...
            {/* Hinglish: strong points section — ab har strength ke saath resume evidence aur JD relevance hai */}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {(analysisData.strongPoints || []).length ? (
                  analysisData.strongPoints.map((item, index) => {
                    // Hinglish: backward compatibility — agar purana plain string format hai toh handle karo
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Hinglish: agar structured data hai toh evidence aur relevance dikhao */}
                        {isStructured && (
            ...
            {/* Hinglish: recommended keywords — JD me hain lekin resume me weak ya missing hain */}
        ...
        {/* ============ GAPS & RISKS TAB ============ */}
        {/* Hinglish: gaps tab — expanded vulnerabilities dikhata hai with full evidence chain */}
        {activeTab === "gaps" && (
              ...
              <div className="space-y-5">
                {(analysisData.vulnerabilities || []).length ? (
                  analysisData.vulnerabilities.map((gap, index) => {
                    // Hinglish: backward compatibility — purane {skill, severity, detail} format ko bhi handle karo
                    ...
                    return (
                      ...
                      >
                        {/* Hinglish: gap ka header — skill name aur severity badge */}
                        <div className="p-6 pb-4">
                          ...
                          {/* Hinglish: agar expanded data hai toh full evidence chain dikhao */}
                          {isExpanded ? (
                            <div className="space-y-3">
                              {/* Hinglish: JD me kya likha hai jo yeh gap create karta hai */}
                              ...
                              {/* Hinglish: resume me is baare me kya hai — ya "Not mentioned" */}
                              <div className="bg-[#1a1a1a] rounded-xl p-4 border border-slate-800">
                              ...
                              {/* Hinglish: yeh gap kyun matter karta hai — real impact */}
                              <div className="bg-[#1a1a1a] rounded-xl p-4 border border-slate-800">
                              ...
                              {/* Hinglish: interviewer kya pooch sakta hai — interview risk */}
                              ...
                              {/* Hinglish: suggested resume fix — exact bullet jo paste kar sako */}
                            ...
                            // Hinglish: fallback — purane format ke liye plain detail dikhao
        ...
        {/* ============ SUGGESTIONS TAB ============ */}
        {/* Hinglish: suggestions tab — structured suggestions with before→after bullet rewrites */}
            ...
            <div className="space-y-5">
              {(analysisData.improvementSuggestions || []).length ? (
                analysisData.improvementSuggestions.map((suggestion, index) => {
                  // Hinglish: backward compatibility — agar purana plain string hai toh simple render karo
                  const isStructured =
                  ...
                  return (
                    ...
                    >
                      {/* Hinglish: suggestion header — section tag aur number */}
                      <div className="p-6 pb-4">
                        ...
                        {/* Hinglish: before → after comparison — current bullet vs improved bullet */}
                        <div className="space-y-3">
                          {/* Hinglish: current bullet — resume me abhi kya likha hai */}
                          <div className="bg-red-500/5 rounded-xl p-4 border border-red-500/20">
                            <p className="text-xs font-bold text-red-400 uppercase tracking-wider mb-1">
                              Current
                            </p>
                          ...
                          {/* Hinglish: arrow dikhao before aur after ke beech */}
                          <div className="flex justify-center">
                            <ArrowRight className="text-emerald-400 rotate-90" size={20} />
                          </div>

                          {/* Hinglish: improved bullet — yeh paste karo resume me */}
                          <div className="bg-emerald-500/5 rounded-xl p-4 border border-emerald-500/20">
                            <p className="text-xs font-bold text-emerald-400 uppercase tracking-wider mb-1">
                              Improved
                            </p>
                          ...
                          {/* Hinglish: reason — yeh change kyun zaroori hai */}
                          <div className="bg-[#1a1a1a] rounded-xl p-3 border border-slate-800">
                            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">
                              Why This Helps
                            </p>
                          ...
                          {/* Hinglish: warning — agar improvement unconfirmed experience assume karta hai */}
                          {suggestion.warning && (
        ...
        {/* ============ IMPROVE RESUME TAB ============ */}
        {/* Hinglish: improve resume tab — AI se improved resume generate karo aur PDF download karo */}
        {activeTab === "improve" && (
          <div className="bg-[#252525] border border-slate-800 p-8 rounded-[2.5rem]">
            <h3 className="text-xl font-bold mb-6 flex items-center gap-2">
            ...
            {!improvedResume ? (
              ...
              <div className="space-y-4">
                {/* Hinglish: improved resume text area — read-only, user dekh sakta hai */}
                ...
                {/* Hinglish: action buttons — PDF download (primary), Copy, Text download (secondary) */}
                <div className="flex gap-3 flex-wrap">
                  {/* Hinglish: PDF download button — primary action, backend se PDF generate karwa ke download karega */}
                  <button
                  ...
                  {/* Hinglish: copy button — clipboard me copy karta hai */}
                  <button
                  ...
                  {/* Hinglish: text download — fallback, plain .txt file download */}
                  <button
